...
internal class PermissionRunner : IWorkerRunner<AuthWorker>
{
    ...
    public async Task InvokeAsync(AuthWorker worker, CancellationToken cancellationToken = default)
    {
        ...
        _subscriber = _redis.Subscribe(_config.Queues[ConfigSettings.QUEUES_PERMISSION_CHANGE_EVENT], (s, o) => UpdatePermissionAsync(o.ToString(), cancellationToken).GetAwaiter().GetResult());
