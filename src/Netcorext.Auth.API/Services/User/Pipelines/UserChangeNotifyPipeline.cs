                                          ...
                                          IRequestPipeline<DeleteUser, Result>
{
    ...
    public async Task<Result<long?>?> InvokeAsync(CreateUser request, PipelineDelegate<Result<long?>> next, CancellationToken cancellationToken = new CancellationToken())
    {
        ...
        if (result == Result.Success && result.Content.HasValue && (request.Roles ?? Array.Empty<CreateUser.UserRole>()).Any())
            await NotifyAsync(_config.Queues[ConfigSettings.QUEUES_USER_ROLE_CHANGE_EVENT], result.Content.Value);

        return result;
    }

    public async Task<Result?> InvokeAsync(UpdateUser request, PipelineDelegate<Result> next, CancellationToken cancellationToken = new CancellationToken())
    {
        var result = await next(request, cancellationToken);
        ...
        if (result == Result.Success && (request.Roles ?? Array.Empty<UpdateUser.UserRole>()).Any())
            await NotifyAsync(_config.Queues[ConfigSettings.QUEUES_USER_ROLE_CHANGE_EVENT], request.Id);

        return result;
    }

    public async Task<Result?> InvokeAsync(DeleteUser request, PipelineDelegate<Result> next, CancellationToken cancellationToken = new CancellationToken())
    {
        var result = await next(request, cancellationToken);
