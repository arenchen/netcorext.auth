...
public class GetPermissionHandler : IRequestHandler<GetPermission, Result<IEnumerable<Models.Permission>>>
{
    ...
    public async Task<Result<IEnumerable<Models.Permission>>> Handle(GetPermission request, CancellationToken cancellationToken = default)
    {
        ...
        if (!request.States.IsEmpty()) predicate = predicate.And(p => request.States.Contains(p.State));
